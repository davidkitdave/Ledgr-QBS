> **Archived 2026-07-01** — Describes the removed `accounting_agents` / `invoice_processing` graph and factory. **Live runtime:** `ledgr_slack` + `ledgr_agent` ([ADR-0032](../0032-ledgr-agent-and-slack-two-packages.md)). History only.

# 0015 — Eval-driven prompt loop (no hardcoded rules, no PII)

> **Superseded for the agent quality gate by [ADR-0033](0033-reference-free-ledgr-agent-eval.md)** (2026-06-29).
> F-cluster pytest gate and `scripts/eval_loop.py` are retired. Principles (no PII, no hardcoded
> doc-kind hacks in extraction) remain valid.

- **Status:** Accepted 2026-06-17 (gate superseded 2026-06-29)
- **Date:** 2026-06-17
- **Deciders:** Ledgr team
