...
sessions: Apex Motor Malaysia SST receipt + a clean Singapore GST invoice.
...
# 2. Playground profile: Acme Auto / MALAYSIA / MYR + COA + Apex Motor entity memory
...
| 2 | **Apex Motor Malaysia receipt** (8% SST line) | `playground_profile.json` (MY) | `accounting_agents_document` | `tax_treatment: SR`, **no** SG 9% flag, `tax_jurisdiction: MALAYSIA`, `account_code: 500-020` |
...
* Apex Motor Malaysia receipt — `scratch/apex_motor_receipt.pdf` is the
...
- [ ] `normalized_invoices[0].lines[0].tax_flagged` = false for a
      mathematically-clean invoice (Apex Motor: 4.81 ≈ 8% of 60.19).
- [ ] `account_code` populated for Apex Motor (was `""` in the c92951d1
...
| `account_code` blank for Apex Motor | `state["coa"]` is empty list | Confirm `playground_profile.json` carries the COA array; restart ADK web after editing the JSON (the load callback reads at session start) |
