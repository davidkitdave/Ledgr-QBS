...
Firestore rejects nested arrays (e.g. table ``rows: list[list[str]]``) with
``Property state contains an invalid nested entity``.
"""
...
def firestore_safe_value(value: Any) -> Any:
    ...
    if isinstance(value, list):
        if value and isinstance(value[0], list):
            # Nested array → list of maps (one row per entry).
