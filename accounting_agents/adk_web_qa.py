...
Why this exists
---------------
The plan's Phase 7 task asks us to "lead ADK web browser QA on YAU LEE". We
can't drive a real browser from a non-interactive session, so this module
performs an equivalent programmatic check: it runs the document lane against
the YAU LEE PDF (via ``document_app``) and prints a structured pass/fail
report for the four ADK web tabs that the user would inspect:
...
async def run_qa(
    ...
    """Drive ``document_app`` on the YAU LEE PDF and gather per-view evidence."""
...
def _render_checklist(report: dict[str, Any], output_path: Path | None) -> None:
    lines: list[str] = []
    lines.append("# ADK Web QA Session — YAU LEE receipt")
    lines.append("")
    ...
    if not st["lines"]:
        ...
        # YAU LEE expectation: SR, not flagged, MYR, account_code populated.
        yau_lee_ok = all(
            ln["tax_treatment"] in ("SR", "SST") and ln["tax_flagged"] is False
            for ln in st["lines"]
        )
    lines.append(
        f"- [x] YAU LEE lines NOT flagged (8% SST, not SG 9%): "
        f"**{'PASS' if yau_lee_ok else 'FAIL'}**"
    )
    lines.append("")
...
def _default_client_state() -> dict:
    """Default state matching the JBI PLUS MY playground profile."""
    return {
...
def main():
    parser = argparse.ArgumentParser(
        description="ADK web QA harness for the document lane (YAU LEE / multi-country)."
    )
    parser.add_argument(
        "--pdf",
