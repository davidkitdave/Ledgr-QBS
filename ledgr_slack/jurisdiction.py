...
1. Adding a row to ``REGION_REGISTRY`` (currency, reference key, tax_system,
   cross_border_flag_policy).
2. Adding reference data under ``ledgr_slack/tax_reference/{yaml_name}``.
"""
...
from ledgr_slack.export.tax_classifier import _bands_active_on, _is_standard_scope
from ledgr_slack.legacy_tax_reference import clear_tax_reference_cache, load_taxonomy
...
# Per-region registry — currency, archived reference key, tax system, cross-border policy.
# WS3 onboarding reads supported_regions().
...
class JurisdictionRule:
    ...
    Attributes:
        ...
        reference_yaml: reference key under ``ledgr_slack/tax_reference/``.
            None for cross-border/ambiguous.
...
def _load_reference(yaml_name: str) -> dict:
    """Load archived jurisdiction reference data (rates + code_map)."""
    try:
        return load_taxonomy(yaml_name)
    except OSError:
        logger.warning("jurisdiction reference not found: %s", yaml_name)
        return {}


def clear_jurisdiction_cache() -> None:
    """Reset cached reference loads. Test helper."""
    clear_tax_reference_cache()
