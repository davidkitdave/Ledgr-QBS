...
class LedgerExporter:
    ...
    def __init__(self, classifier: Optional[TaxClassifier] = None):
        # Live Slack path passes pre-built rows; no TaxClassifier on hot path.
        self._classifier = classifier
        self._coa_keys: set[str] | None = None
        self._client_tax_codes: list[dict] | dict[str, str] | None = None

    @property
    def clf(self) -> TaxClassifier:
        """Lazy legacy classifier — not loaded until row export runs."""
        if self._classifier is None:
            self._classifier = TaxClassifier()
