...
Legacy-only: live Slack delivery uses ``ledgr_agent`` skill profiles. Taxonomy
data is under ``ledgr_slack/tax_reference/`` and loaded only when a
:class:`TaxClassifier` is explicitly constructed (not on ``get_exporter()``).
"""
...
from ledgr_slack.legacy_tax_reference import _TAX_REF_DIR, load_keyword_aliases, load_taxonomy
...
def get_tax_classifier(reference_yaml: Optional[str] = None) -> Optional["TaxClassifier"]:
    ...
    yaml_path = _TAX_REF_DIR / ref
    if not yaml_path.exists():
        logger.warning(
            "get_tax_classifier: '%s' not found in ledgr_slack/tax_reference",
            ref,
        )
        return None
