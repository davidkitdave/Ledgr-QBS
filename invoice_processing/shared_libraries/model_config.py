...
All application code should import model helpers from this module (or re-exports
in ``accounting_agents.config`` / ``genai_client``). Do not read ``LEDGR_MODEL_*``
env vars elsewhere.
...
def resolve_model(tier: str) -> str:
  ...
  Reads ``LEDGR_MODEL_<TIER>`` from the environment when set; otherwise returns
  the repo default. For ``std`` only, falls back to legacy ``GEMINI_FLASH_MODEL``.
    """
